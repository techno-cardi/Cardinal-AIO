...
(async () => {
...

  {
    ...
    assert(text.includes('declaredTotalPoints={value,provenance}'));
    assert(text.includes('{id,role,label,status}'));
    assert(text.includes('chaque bonne option DOIT avoir points explicite'));
    assert(text.includes('sinon partialCredit=false'));
    assert(text.includes('libellé humain visible autour de chaque placeholder'));
    assert(text.includes('providedAnswerKey/sourceExplicit/sourceInferred/questionIntrinsic/teacherApproved/sourceMissing'));
    assert(text.includes('N’utilise jamais type à la place de code'));
