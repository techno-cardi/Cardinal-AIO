(() => {
  ...
  const MAX_PREPARE_TEXT_CHARS = 6500;
  const COMPACT_PROTOCOL = [
    ...
    '- Avant de produire le corrigé, établis les critères de réussite à partir de la consigne, des sources, du corrigé, du barème et des exemples fournis.',
    '- Corrigé/barème/exemples fournis par l’enseignant = référence pédagogique prioritaire. Si cette référence contredit clairement la consigne ou une source vérifiable, signale le conflit au lieu de trancher silencieusement.',
    '- Juge le sens et l’équivalence sémantique plutôt que la présence mécanique d’un mot, sauf si la formulation exacte est réellement évaluée.',
    '- Réponses identiques ou sémantiquement équivalentes => traitement cohérent.',
    '- Question multipartie: évalue chaque élément séparément; ne compte pas deux fois la même idée.',
    '- Ambiguïté raisonnable dans la consigne: ne pénalise pas l’élève pour une interprétation défendable.',
    '- Si une source nécessaire manque, ne conclus pas définitivement et signale le blocage. N’invente jamais fait, citation, page, ligne, réponse, distracteur ou pondération.',
    '- Fais une deuxième passe silencieuse de cohérence avant la sortie et vérifie les angles morts pédagogiques: sous-parties oubliées, consigne implicite, source manquante, réponse équivalente, négation, relation cause/conséquence, total, crédit partiel et ambiguïté.',
    '',
    'SORTIE OBLIGATOIRE',
    '1) un tableau humain compact pour vérifier questions, type, points, correction et problèmes;',
    '2) une ligne exacte CARDINAL_FORMATIVE_PACKAGE_V2;',
    '3) un seul bloc JSON contenant un seul objet JSON, sans deuxième paquet.',
    '',
    'PAQUET',
    'schema="cardinal.formative/2"; protocolVersion="2.0.0"; packageMode="full" sauf patch demandé; assessment, sources, items, issues.',
    'assessment: title, language="fr-CA", sourceMode="external-reference-only" ou "embedded" si passageGroup, declaredTotalPoints={value,provenance} si connu; jamais un nombre seul.',
    'Sources: seulement celles réellement utiles; forme {id,role,label,status}; roles questionnaire/text/answerKey/rubric/appendix/unknown; status provided/missing/external. N’utilise pas description à la place de label.',
    'Items: section, instruction ou question; id/order uniques. Question: source{sourceRef,page,printedPage,number,subNumber,promptExact}, sourceRefs,prompt,subtype,required(bool),points{value,provenance,graded:true,bonus:false},grading,transformations,issues.',
    '',
    'TYPE IDÉAL D’ABORD',
    'choix unique=multipleChoice; choix multiples=multipleSelection; menu/catégorie par champ=inlineChoice; association=matching; ordre=resequence; champs texte déterministes=fillInTheBlank; réponse courte unique=shortAnswer; justification/raisonnement/rédaction=longAnswer.',
    'PROVEN ici: shortAnswer,longAnswer,fillInTheBlank,multipleChoice,multipleSelection,inlineChoice,resequence,matching.',
    'Tout autre subtype reste bloqué sauf fallback PROVEN réellement équivalent, déclaré avec idealSubtype+compromis+requiresReview=true. Ne convertis pas un type structuré en longAnswer seulement pour l’import; ne remplace pas un dropdown fermé par un champ texte.',
    'Structures: QCM=>response.options[{id,text,correct,points?}]. multipleSelection avec partialCredit=true: chaque bonne option DOIT avoir points explicite et leur somme DOIT égaler points.value; sinon partialCredit=false, jamais de pondération arbitraire. inlineChoice=>{{id}}+response.dropdowns[{id,options,correct:[valeur]}], avec un libellé humain visible autour de chaque placeholder; jamais {{a}} {{b}} seuls. fillInTheBlank=>{{id}}+response.blanks[{id,answers,conceptIds:[]}]. resequence=>response.sequence[texte]. matching=>response.pairs[{left,right}].',
    'Question mixte: découpe seulement si la source/barème permet une répartition de points certaine; sinon garde-la groupée. Conserve options, bonnes réponses, ordre, points, crédit partiel et total. Si total incohérent: issue TOTAL_POINTS_MISMATCH.',
    '',
    'CORRIGÉ',
    'grading={mode,expectedAnswer,provenance:{kind,sourceRefs},partialCredit(bool),caseSensitive(bool),requirements,concepts}. provenance.kind seulement: providedAnswerKey/sourceExplicit/sourceInferred/questionIntrinsic/teacherApproved/sourceMissing; jamais "answerKey".',
    'Corrigé fourni > source explicite > inférence prudente > réponse intrinsèque à la question > aucune invention.',
    'Pour short/long auto ou assisted: concepts[{id,label,score,provenance,terms,riskyTerms}], avec 2-5 termes discriminants réels par idée si disponibles; évite mots génériques et faux synonymes.',
    'Keyword: score absolu par match, jamais cumulatif. Un seul mot ne vaut pas toute une réponse complexe. Négation, plusieurs éléments, justification ou relation => assisted + requirements + scores prudents. manual seulement si une correction fiable est impossible. expectedAnswer reste complet en assisted.',
    '',
    ...
    'Source/média absent: provenance.kind="sourceMissing", issue SOURCE_REQUIRED ou MEDIA_DEPENDENCY_MISSING; blocker si inutilisable, sinon warning. Cardinal respecte ce jugement.',
    '',
    'CONTRÔLE FINAL',
