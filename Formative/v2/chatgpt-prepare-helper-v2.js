(() => {
  ...
  const COMPACT_PROTOCOL = [
    ...
    'assessment: title, language="fr-CA", sourceMode="external-reference-only", declaredTotalPoints si connu.',
    'Sources: seulement celles réellement utiles; roles questionnaire/text/answerKey/rubric/appendix/unknown; status provided/missing/external.',
    ...
    'Structures: QCM=>response.options[{id,text,correct,points?}]. inlineChoice=>{{id}}+response.dropdowns[{id,options,correct:[valeur]}]. fillInTheBlank=>{{id}}+response.blanks[{id,answers,conceptIds:[]}]. resequence=>response.sequence[texte]. matching=>response.pairs[{left,right}].',
    ...
    '',
    'CORRIGÉ',
    'grading={mode,expectedAnswer,provenance:{kind,sourceRefs},partialCredit,caseSensitive,requirements,concepts}.',
    ...
    'Conserve source.promptExact. prompt est la version Formative; retire seulement la numérotation source sans changer le fond. Toute transformation de fond => splitQuestion/changeResponseType/other + description + requiresReview=true.',
